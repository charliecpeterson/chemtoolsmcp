...
| `chemtools/execution/targets.py` | Loads schema-2 YAML or JSON into immutable named local and Slurm targets with explicit execution permission and default selection | Target configuration adapter | Keep arbitrary commands and roots in host-owned configuration. Guided NWChem can use this path without loading a version 1 profile. |
