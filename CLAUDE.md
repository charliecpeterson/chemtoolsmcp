...
- **MPI needs a big CI; tiny references can't parallelize**: dogfooding neutral Th 6d²7s² (9 CSFs) showed `rmcdhf_mpi` can't solve the diffuse 7s (any start: TF / screened-hydrogenic / hf-bootstrap; any np) though serial `rmcdhf` can, and `rci_mpi` aborts `matrix: too many nodes` because blocks are 1–3 CSFs (< np). The robust pattern for neutral heavy atoms is **serial SCF + `rci_mpi` on a *large* SD/MR expansion** (RCI has no radial-orbital solver). `rci` needs an `rmcdhf`-processed `.w`, not a raw `rwfnestimate` file, so estimated correlation orbitals are passed through an `rmcdhf`-vary-nothing step (0 SCF iters) first. Working example: `input_examples/grasp/Th_MPI_RCI` (~1250-CSF Th RCI at np=4). The orbital-solver crash is caught (see `parse/rmcdhf_log.py`).
