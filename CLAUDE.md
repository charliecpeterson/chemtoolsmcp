...
- Current tool count: 169 (114 NWChem + 40 Molcas + 15 generic — Phases 4 / 4b / 4c / 5; the NWChem total includes `get_server_mode`)
...
|---|---|---|
| `analysis` | 152 | No NWChem executable available; post-hoc parsing (NWChem + Molcas), drafting (incl. Molcas inputs), planning, registry tracking of runs done elsewhere |
| `local` | 166 | NWChem runs as a subprocess on this machine (profile with `launcher.kind: "direct"`) |
| `hpc` | 169 | NWChem submitted to a scheduler (profile with `launcher.kind: "scheduler"`) |
