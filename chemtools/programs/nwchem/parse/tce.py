...
"""TCE (Tensor Contraction Engine) text-output support for NWChem.

Covers:
  - parse_tce_output: extract energies, convergence, frozen orbital info
  - parse_tce_amplitudes: read T1/D1 / MR diagnostics from amplitude files
  - suggest_tce_freeze_count: chemically-aware core orbital count
    (never freeze atomic)
  - analyze_tce_orbital_ordering: warn when orbital ordering may require
    swaps before TCE

The binary-format movecs reader/writer (parse_nwchem_movecs,
swap_nwchem_movecs, _read_fortran_records, etc.) lives in
chemtools/programs/nwchem/binary/movecs.py. The relevant names are
re-exported at the bottom of this module for back-compat with callers
that still import them from `parse.tce`.
"""
...
# ---------------------------------------------------------------------------
# Binary movecs IO moved to chemtools/programs/nwchem/binary/movecs.py
# Re-exported here for back-compat. New callers should import directly from
# binary/movecs.
# ---------------------------------------------------------------------------
from chemtools.programs.nwchem.binary.movecs import (  # noqa: F401, E402
    parse_nwchem_movecs,
    swap_nwchem_movecs,
)
