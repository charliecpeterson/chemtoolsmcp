...
| Phase 3: framework promotion | **DONE 2026-05-12** — `core/issues.py` (IssueCollector + severity tracking), `core/recovery.py` (generic rule-walker `dispatch_rules`), `core/case_analysis.py` (`classify_imaginary_modes`, `check_charge_spin_parity`, `bond_table_for_atoms`). Molcas `recovery.py` + `case_analysis.py` thinned to thin adapters. Completed in Phase 6b: `core/session.py`, `core/basis_advisor.py`, `core/workflow.py`. |
| Phase 4: generic MCP tools | **DONE 2026-05-12** — 5 new generic tools landed: `extract_geometry`, `parse_thermochem`, `parse_frequencies`, `parse_trajectory`, `inspect_geometry`. Each dispatches via `registry.resolve(path)` to the appropriate plugin's `parser.get_*()` method. Tool count: 159 (was 154). Mode counts: 142/156/159 (all 5 are `needs="none"`). Molcas plugin's `get_geometry` now normalizes bohr→Å so generic dispatchers don't need program-specific unit logic. NWChem-side fixture geometries are sparse (only opt+freq outputs have atoms); validated cross-program via Molcas H2O opt+freq (formula H2O, 2 covalent O-H bonds, 1 HOH angle). Tests: 32 total (was 30) — 2 new for Phase-4 tagging + auto-dispatch. The `summarize_output` / `analyze_case` / `suggest_recovery` / `apply_recovery` generics that were originally deferred from this phase shipped in Phase 6a. |
...
| Phase 4c: legacy registry tools generalized | **DONE 2026-05-12** — 8 new generic MCP registry tools alongside their legacy NWChem-prefixed counterparts: `update_run_status`, `list_runs` (with `program` filter), `get_run_summary`, `create_campaign`, `get_campaign_status`, `get_campaign_energies` (program per row), `create_workflow`, `advance_workflow`. All tagged `program='generic'`, `needs='registry'`. Legacy `*_nwchem_*` aliases preserved for backward compat — `list_nwchem_runs` auto-pre-fills program='nwchem' when no explicit filter passed. Tool count: 168 (was 160). 34 tests pass. |
| Phase 5: standardized parse shape | **DONE 2026-05-12** — ParsedRun top-level keys already aligned by the plugin Protocol from Phase 1 of the original refactor. Added (a) a generic `parse_output` MCP tool (program='generic', no needs) that calls `plugin.parser.parse_output()` and returns the unified ParsedRun for any program. (b) cross-program key aliasing in `derived`: NWChem `final_energy_hartree` now also surfaces as `primary_energy_hartree`; Molcas `primary_energy_hartree` now also surfaces as `final_energy_hartree`. Generic tools that read `parsed["derived"]["final_energy_hartree"]` work uniformly across programs. The program-prefixed `parse_nwchem_output` / `parse_molcas_output` are deliberately NOT unified — they're the deep parsers, intended for program-specific rich data (MO blocks, per-module CASPT2 details, NWChem MCSCF block, etc.). Tool count: 169 (was 168). 35 tests pass. |
| Phase 6: parity fill-in | tracked above |
| Phase 6a: case-analysis + recovery dispatchers | **DONE 2026-05-12** — 4 new generic MCP tools (`summarize_output`, `analyze_case`, `suggest_recovery`, `apply_recovery`) that auto-detect via `registry.resolve()` and route to the program-specific implementation (`summarize_nwchem_output` / `summarize_molcas_output`, etc.). Returns the per-program shape tagged with `program:` so the agent can branch its own follow-up logic. Decision: NOT forcing the two programs to a unified shape — NWChem's `summarize_nwchem_case` returns rich state-check + active-space density payloads; Molcas's returns flat freq/thermo summary; both useful where they differ. `apply_recovery` returns `verdict='not_implemented_for_program'` for NWChem (no mechanical patcher exists yet). Tool count: 173 (was 169). 38 tests pass — 3 new for the dispatcher behaviors. |
| Phase 6b: session log + basis advisor + workflow DAG → core/ | **DONE 2026-05-12** — three internal relocations, zero MCP surface changes. (a) `core/session.py` houses `init_session_log` / `append_session_log` / `next_versioned_path` (originally in `programs/nwchem/runner.py:1430-1503`); `runner.py` re-exports for backward compat; `api.py` imports from core directly. (b) `core/basis_advisor.py` houses `suggest_basis_set` (originally in `programs/nwchem/strategy/input_advisors.py:187`); local `_TRANSITION_METALS` frozenset removes NWChem dependency; `input_advisors.py` re-exports. (c) `core/workflow.py` houses the generic DAG engine (`list_protocols(protocols)` + `plan_calculation(protocols, ..., tool_names, dynamic_generators)`); `programs/nwchem/protocols.py` becomes a thin wrapper that injects `PROTOCOLS`, the NWChem tool-name mapping (`check_nwchem_freq_plausibility`, `launch_nwchem_run`, etc.), and the spin-scan + reaction-species step generators. A future Molcas protocol library plugs into the same engine the same way. Tool count unchanged (173 NWChem + 40 Molcas = 213). All 244 phase tests + 38 mode tests pass. |
| Phase 6c: post-refactor cleanup | **DONE 2026-05-12** — closed the audit punch-list: (a) **Quick wins** — fixed `test_phase1/test_all.py` imports from the pre-split `chemtools.runner` (now `chemtools.core.runner`); auto-generated `.fdrst` / `.db` / `.jobid` test fixtures; reconciled CLAUDE.md tool-count breakdown (97 NWChem + 40 Molcas + 36 generic = 173); cleared stale "deferred" lines in REFACTOR_CORE.md; rewrote misleading docstring in `programs/nwchem/strategy/input_advisors.py`. Surfaced 4 latent bugs that the broken import had been masking: `_BASIS_SCALE` unresolved in `programs/nwchem/strategy/resources.py`, and missing `parse_start_blocks` / `inspect_nwchem_module_vectors` / `_TRANSITION_METALS` imports in `programs/nwchem/input/lint_restart.py`. (b) **Direct unit tests for core/*** — added `test_phase1/test_core.py` with 49 tests directly exercising `core/{units,thermochem,geometry,issues,recovery,case_analysis,session,basis_advisor,workflow}.py` (previously exercised only transitively via MCP-layer tests). (c) **Layering violation fixed** — extracted ~316 lines of NWChem-specific progress / slow-phase / requested-task helpers from `core/runner.py` (1432 → 1102 LOC) into `programs/nwchem/strategy/progress.py`. `core/runner.py` no longer has any static `from chemtools.programs.*` imports — the NWChem progress builder is lazy-loaded only after `detect_program(contents) == "nwchem"`. Verified zero static `programs/` imports in `core/`. All 364 tests pass (244 phase + 33 all + 38 modes + 49 new core). |
